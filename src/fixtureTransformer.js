...
export function buildFixture(input) {
  ...
  var flowVolumeMetaByLabel = new Map(); // <— NEW: keep unit/interval for preexisting volume
  // Cleaned metrics (outputs of cleaning triggers)
  var cleanedFlowReadingByLabel = new Map();
  var cleanedPressureReadingByLabel = new Map();
  var cleanedFlowVolumeByLabel = new Map(); // <— NEW
  ...
  for (var _i = 0, input_1 = input; _i < input_1.length; _i++) {
    ...
    if (cat === "zone") {
      ...
      var fields = {
        ...
        coords: JSON.stringify(item.coords, null, 2),
        category: "zone",
        attribute:
          (_b = item.attribute) !== null && _b !== void 0
            ? _b
            : {
                metadata: [],
      ...
      var pointArray = Array.isArray(item.coords) ? item.coords : [null, null];
      var coordsString =
        pointArray.length >= 2 &&
      ...
      var fields = {
        ...
        category: cat,
        attribute:
          (_d = item.attribute) !== null && _d !== void 0
            ? _d
            : {
                metadata: [],
  ...
  for (var _h = 0, input_2 = input; _h < input_2.length; _h++) {
    ...
    var unit = (
      (_e = item.metric_unit) !== null && _e !== void 0 ? _e : defaultUnit
    ).toString();
    var interval = (
      (_f = item.metric_interval) !== null && _f !== void 0
        ? _f
        : "QUARTER_HOUR"
    ).toString();

    var FRIENDLY_LABEL = {
      flow_reading: "Readings",
      pressure_reading: "Pressure",
      flow_volume: "Flow Volume",
      zone_demand: "Zone Demand",
      average_zone_pressure: "Average Zone Pressure",
    };
    var metricLabel =
      (FRIENDLY_LABEL[metricCategory] || metricCategory) + " - " + item.label;
    ...
    var storage_table =
      (_g = item.metric_storage_table) !== null && _g !== void 0
        ? _g
        : DEFAULT_STORAGE_TABLE;
    if (storage_table == null) {
      throw new Error(
        'Metric for "'.concat(item.label, '" is missing storage_table. ') +
    ...
    if (metricCategory === "flow_reading") {
      ...
      flowReadingMetaByLabel.set(item.label, {
        unit: unit,
        interval: interval,
      });
    } else if (metricCategory === "flow_volume") {
      flowVolumeMetricByLabel.set(item.label, pk);
      flowVolumeMetaByLabel.set(item.label, { unit: unit, interval: interval }); // <— NEW
    } else if (metricCategory === "pressure_reading") {
      pressureReadingMetricByLabel.set(item.label, pk);
      pressureReadingMetaByLabel.set(item.label, {
        unit: unit,
        interval: interval,
      });
    }
  ...
  // ---------- 2.1) Cleaners: sensor flow/pressure ----------
  var CLEAN_CALC_NAME = "CLEAN_DATA";

  flowReadingMetricByLabel.forEach(function (rawMetricId, lbl) {
    var meta = flowReadingMetaByLabel.get(lbl) || {
      unit: "m³",
      interval: "QUARTER_HOUR",
    };
    var targetId = labelToTargetId.get(lbl);
    if (!targetId) return;

    var cleanMetricId = stableId(PREFIX.metric, [
      "clean_flow_reading",
      lbl,
      meta.unit,
      meta.interval,
    ...
    var trigId = stableId(PREFIX.trigger, ["clean_flow_reading", lbl]);
    rows.push({
      model: MODEL.Trigger,
      pk: trigId,
      fields: {
        ...
        calculators: [
          {
            calc_args: {
              ...
              inputs: {},
              metric_id: stableUuid(["clean_flow_reading", lbl]),
            },
            calc_name: CLEAN_CALC_NAME,
          },
        ],
        description: "Readings (Cleaned) - " + lbl,
        schedule_job: "",
        input_metrics: [rawMetricId],
        output_metric: cleanMetricId,
      },
    });

    cleanedFlowReadingByLabel.set(lbl, cleanMetricId);
  });

  pressureReadingMetricByLabel.forEach(function (rawMetricId, lbl) {
    var meta = pressureReadingMetaByLabel.get(lbl) || {
      unit: "bar",
      interval: "QUARTER_HOUR",
    };
    var targetId = labelToTargetId.get(lbl);
    ...
    var trigId = stableId(PREFIX.trigger, ["clean_pressure_reading", lbl]);
    rows.push({
      model: MODEL.Trigger,
      pk: trigId,
      fields: {
        ...
        calculators: [
          {
            calc_args: {
              ...
              time_range_type: "operation",
              inputs: {},
              metric_id: stableUuid(["clean_pressure_reading", lbl]),
            },
            calc_name: CLEAN_CALC_NAME,
          },
        ],
        description: "Pressure (Cleaned) - " + lbl,
        schedule_job: "",
        input_metrics: [rawMetricId],
        output_metric: cleanMetricId,
      },
    ...
    cleanedPressureReadingByLabel.set(lbl, cleanMetricId);
  });

  // ---------- 2.2) Cleaners: pre-existing flow_volume (from input) ----------
  // Run this BEFORE we auto-create flow_volume from readings (section 4)
  flowVolumeMetricByLabel.forEach(function (rawVolMetricId, lbl) {
    var meta = flowVolumeMetaByLabel.get(lbl) || {
      unit: "m³",
      interval: "QUARTER_HOUR",
    };
    var targetId = labelToTargetId.get(lbl);
    ...
    var cleanVolMetricId = stableId(PREFIX.metric, [
      "clean_flow_volume",
      lbl,
      meta.unit,
      meta.interval,
    ...
    rows.push({
      ...
      pk: cleanVolMetricId,
      fields: {
        label: "Flow Volume (Cleaned) - " + lbl,
        category: "flow_volume", // keep same category; mark cleaned via tags/source
        unit: meta.unit,
        interval: meta.interval,
        tags_list: '["cleaned"]',
        target_id: targetId,
    ...
    var trigId = stableId(PREFIX.trigger, ["clean_flow_volume", lbl]);
    rows.push({
      model: MODEL.Trigger,
      pk: trigId,
      fields: {
        ...
        calculators: [
          {
            calc_args: {
              time_range_type: "operation",
              inputs: {},
              metric_id: stableUuid(["clean_flow_volume", lbl]),
            },
            calc_name: CLEAN_CALC_NAME,
          },
        ],
        description: "Cleaned Flow Volume - " + lbl,
        schedule_job: "",
        input_metrics: [rawVolMetricId],
        output_metric: cleanVolMetricId,
      },
    });

    cleanedFlowVolumeByLabel.set(lbl, cleanVolMetricId);
  });
  ...
  for (var _j = 0, input_3 = input; _j < input_3.length; _j++) {
    ...
    var relationType =
      cat === "flow_meter"
        ? "flow_meter_in_zone"
        : cat === "pressure_sensor"
        ? "pressure_sensor_in_zone"
        : "device_in_zone";

    if (item.inletFor && zoneLabels.has(item.inletFor)) {
      var zoneIdIn = labelToTargetId.get(item.inletFor);
      rows.push({
        model: MODEL.Link,
        pk: nextLinkPk++,
        fields: {
    ...
    if (item.outletFor && zoneLabels.has(item.outletFor)) {
      var zoneIdOut = labelToTargetId.get(item.outletFor);
      rows.push({
        model: MODEL.Link,
        pk: nextLinkPk++,
        fields: {
  ...
  flowReadingMetricByLabel.forEach(function (readingMetricId, lbl) {
    // If a flow_volume metric was supplied in input, skip creating a duplicate
    if (flowVolumeMetricByLabel.has(lbl)) {
      return; // Zone Demand will see cleanedFlowVolumeByLabel if present
    }
    var meta = flowReadingMetaByLabel.get(lbl) || {
      unit: "m³",
      interval: "QUARTER_HOUR",
    ...
    // Input = CLEANED flow readings
    var trigId = stableId(PREFIX.trigger, ["flow_volume", lbl]);
    var cleanedInput = cleanedFlowReadingByLabel.get(lbl) || readingMetricId;
    rows.push({
      model: MODEL.Trigger,
      pk: trigId,
      fields: {
        ...
        schedule_job: "",
        input_metrics: [cleanedInput],
        output_metric: volumeMetricId,
      },
    });
  });

  // Build zone → inputs (prefer CLEANED flow_volume and CLEANED pressure)
  ...
  zoneLabels.forEach(function (zlbl) {
    ...
    pointCoordsByLabel.forEach(function (pt, lbl) {
      ...
      var fvClean = cleanedFlowVolumeByLabel.get(lbl);
      var fvRaw = flowVolumeMetricByLabel.get(lbl);
      var fv = fvClean || fvRaw;
      if (fv) flowVols.push(fv);

      var prClean = cleanedPressureReadingByLabel.get(lbl);
      if (prClean) pressReads.push(prClean);
    });
  ...
  zoneToFlowVolumeInputs.forEach(function (inputMetricIds, zlbl) {
    ...
    rows.push({
      ...
      fields: {
        ...
        description: "".concat("Demand - ", zlbl),
        schedule_job: "",
